...
if (archiveCarousel) {
  ...
  const archivePrevButton = archiveCarousel.querySelector("[data-archive-prev]");
  const archiveNextButton = archiveCarousel.querySelector("[data-archive-next]");
  ...
  archiveCards.forEach((card, index) => {
    card.addEventListener("click", () => {
      ...
      const targetSelector = card.dataset.target?.trim();
  ...
  archivePrevButton?.addEventListener("click", () => updateArchive(activeArchiveIndex - 1));
  archiveNextButton?.addEventListener("click", () => updateArchive(activeArchiveIndex + 1));
...
/* WORK ARCHIVE STICKY MENU */
const archiveStickyNav = document.querySelector("[data-archive-sticky-nav]");

if (archiveStickyNav) {
  const stickyLinks = [...archiveStickyNav.querySelectorAll("[data-archive-jump]")];
  const archiveSections = [...document.querySelectorAll("[data-archive-section]")];
  const archiveMenuToggle = archiveStickyNav.querySelector("[data-archive-menu-toggle]");
  const archiveMenuList = archiveStickyNav.querySelector("[data-archive-menu-list]");
  const archiveMenuLabel = archiveStickyNav.querySelector("[data-archive-menu-label]");

  const closeArchiveMenu = () => {
    if (!archiveMenuToggle || !archiveMenuList) return;
    archiveMenuToggle.setAttribute("aria-expanded", "false");
    archiveMenuToggle.setAttribute("aria-label", "워크 아카이브 메뉴 열기");
    archiveMenuList.hidden = true;
  };

  archiveMenuToggle?.addEventListener("click", () => {
    if (!archiveMenuList) return;
    const willOpen = archiveMenuToggle.getAttribute("aria-expanded") !== "true";
    archiveMenuToggle.setAttribute("aria-expanded", String(willOpen));
    archiveMenuToggle.setAttribute(
      "aria-label",
      willOpen ? "워크 아카이브 메뉴 닫기" : "워크 아카이브 메뉴 열기",
    );
    archiveMenuList.hidden = !willOpen;
  });

  const setActiveArchiveJump = (index) => {
    stickyLinks.forEach((link) => {
      const isActive = Number(link.dataset.archiveJump) === index;
      link.classList.toggle("is-active", isActive);
      if (isActive) link.setAttribute("aria-current", "true");
      else link.removeAttribute("aria-current");

      if (isActive && archiveMenuLabel) {
        const number = link.querySelector("span")?.textContent?.trim() || "";
        const title = link.querySelector("strong")?.textContent?.trim() || "";
        archiveMenuLabel.textContent = `${number} · ${title}`;
      }
    });
  };
  ...
  stickyLinks.forEach((link) => {
    link.addEventListener("click", () => {
      setActiveArchiveJump(Number(link.dataset.archiveJump));
      closeArchiveMenu();
    });
  });

  document.addEventListener("click", (event) => {
    if (!archiveStickyNav.contains(event.target)) closeArchiveMenu();
  });
  ...
  const archiveSectionObserver = new IntersectionObserver(
    (entries) => {
      ...
      if (visibleEntry) {
        setActiveArchiveJump(Number(visibleEntry.target.dataset.archiveSection));
      }
    },
...
if (posterMarquee && posterModal) {
  ...
  const measurePosterMarquee = () => {
    ...
    if (!marqueeInitialized || previousCycleWidth === 0) {
      posterPosition = centerStart;
      posterViewport.scrollLeft = posterPosition;
    ...
    posterPosition = centerStart + relativeProgress * cycleWidth;
    posterViewport.scrollLeft = posterPosition;
  };

  const setPosterPosition = (nextPosition) => {
    posterPosition = nextPosition;

    if (cycleWidth) {
      while (posterPosition >= centerStart + cycleWidth) {
        posterPosition -= cycleWidth;
      }
      while (posterPosition <= centerStart - cycleWidth) {
        posterPosition += cycleWidth;
      }
  ...
  posterViewport.addEventListener("pointerdown", (event) => {
    ...
    posterPosition = posterScrollStart;
  ...
  posterViewport.addEventListener("pointermove", (event) => {
    ...
    setPosterPosition(posterScrollStart - posterDragDistance);
  });
  ...
  const animatePosterMarquee = (time) => {
    ...
    if (shouldMove) {
      setPosterPosition(posterPosition + posterAutoSpeed * delta);
    }
  ...
  window.addEventListener("load", measurePosterMarquee, { once: true });
...
// HERO BANNER DESIGN SLIDER
const heroBannerSlider = document.querySelector("[data-hero-banner-slider]");

if (heroBannerSlider) {
  const viewport = heroBannerSlider.querySelector(".hero-banner-slider__viewport");
  const slides = [...heroBannerSlider.querySelectorAll("[data-hero-banner-slide]")];
  const navButtons = [...heroBannerSlider.querySelectorAll("[data-hero-banner-nav]")];
  ...
  const updateHeroBanner = (nextIndex, announce = true) => {
    ...
    if (announce && status) {
      status.textContent = `${slides[activeIndex].getAttribute("aria-label")} 표시 중`;
    }
  };
  ...
  viewport?.addEventListener("keydown", (event) => {
    if (event.key === "ArrowLeft") updateHeroBanner(activeIndex - 1);
    if (event.key === "ArrowRight") updateHeroBanner(activeIndex + 1);
  });
...
// BRAND CONTENT MODAL
const brandModal = document.querySelector("[data-brand-modal-dialog]");

if (brandModal) {
  // 모달 이미지 교체 방법:
  // 아래 images 배열의 경로를 새 이미지 경로로 바꾸거나,
  // 같은 파일명으로 images 폴더의 파일을 교체하면 됩니다.
  const brandProjects = {
    "nice-01": {
      eyebrow: "NICE KITCHEN / GOODS",
      title: "HOME SWEET HOME RUG",
      description: "브랜드의 유쾌한 분위기를 일상에서도 경험할 수 있도록 제작한 패브릭 제품입니다.",
      images: ["./images/brand-nice-01.png"],
    },
    "nice-02": {
      eyebrow: "NICE KITCHEN / POP-UP",
      title: "GOODS DISPLAY",
      description: "다양한 판매 제품이 한눈에 보이도록 팝업 공간의 진열 장면과 제품 구성을 정리했습니다.",
      images: ["./images/brand-nice-02.png"],
    },
    "nice-03": {
      eyebrow: "NICE KITCHEN / PRINT",
      title: "POSTCARD DISPLAY",
      description: "브랜드의 그래픽과 메시지가 오프라인 공간에서도 자연스럽게 이어지도록 엽서 제품을 제작했습니다.",
      images: ["./images/brand-nice-03.png"],
    },
    "nice-04": {
      eyebrow: "NICE KITCHEN / GOODS",
      title: "CHECKERBOARD GOODS",
      description: "선명한 컬러와 체커보드 패턴을 활용해 브랜드의 경쾌한 인상을 담은 생활 제품을 제작했습니다.",
      images: ["./images/brand-nice-04.png"],
    },
    "nice-postcards": {
      eyebrow: "NICE KITCHEN / PRINT",
      title: "POSTCARD COLLECTION",
      description: "브랜드의 위트 있는 메시지와 그래픽을 담아 판매 및 배포용 엽서 컬렉션을 디자인했습니다.",
      images: [
        "./images/brand-postcard-01.png",
        "./images/brand-postcard-02.png",
        "./images/brand-postcard-03.png",
      ],
    },
    "allforhome-social": {
      eyebrow: "ALLFORHOME / SOCIAL CONTENT",
      title: "SOCIAL CAMPAIGN",
      description: "제품의 특징과 프로모션 혜택이 빠르게 전달되도록 이미지와 문구의 우선순위를 정리해 SNS 콘텐츠를 제작했습니다.",
      layout: "social-grid",
      images: [
        "./images/brand-allforhome-01.png",
        "./images/brand-allforhome-02.png",
        "./images/brand-allforhome-03.png",
        "./images/brand-allforhome-04.png",
      ],
    },
    "allforhome-brochure": {
      eyebrow: "ALLFORHOME / EDITORIAL",
      title: "BRAND BROCHURE",
      description: "제품 정보와 브랜드의 생활 장면이 자연스럽게 이어지도록 이미지와 내용을 정리해 브랜드 인쇄물을 제작했습니다.",
      images: ["./images/brand-allforhome-brochure.png"],
    },
    "rawumber-postcards": {
      eyebrow: "RAWUMBER / PRINT",
      title: "POSTCARD COLLECTION",
      description: "계절마다 달라지는 브랜드의 분위기를 그래픽과 타이포그래피로 표현해 배포용 엽서 시리즈를 디자인했습니다.",
      layout: "grid-2",
      images: [
        "./images/brand-rawumber-postcard-01.png",
        "./images/brand-rawumber-postcard-02.png",
        "./images/brand-rawumber-postcard-03.png",
        "./images/brand-rawumber-postcard-04.png",
      ],
    },
    "rawumber-campaign": {
      eyebrow: "RAWUMBER / CAMPAIGN",
      title: "POP-UP CAMPAIGN",
      description: "팝업 행사 일정과 시즌 제품이 한눈에 전달되도록 사진과 일러스트를 활용해 홍보 이미지를 제작했습니다.",
      images: [
        "./images/brand-rawumber-campaign-01.png",
        "./images/brand-rawumber-campaign-02.png",
      ],
    },
    "rawumber-order": {
      eyebrow: "RAWUMBER / PRINT",
      title: "POP-UP ORDER FORM",
      description: "팝업 현장에서 다양한 제품과 옵션을 빠르게 확인하고 주문할 수 있도록 정보를 체계적으로 정리한 주문서를 디자인했습니다.",
      images: ["./images/brand-rawumber-order.png"],
    },
  };

  const gallery = brandModal.querySelector("[data-brand-modal-gallery]");
  const eyebrow = brandModal.querySelector("[data-brand-modal-eyebrow]");
  const title = brandModal.querySelector("[data-brand-modal-title]");
  const description = brandModal.querySelector("[data-brand-modal-description]");
  const closeButton = brandModal.querySelector("[data-brand-modal-close]");
  let lastBrandTrigger = null;

  const openBrandModal = (projectKey, trigger) => {
    const project = brandProjects[projectKey];
    if (!project) return;

    lastBrandTrigger = trigger;
    gallery.replaceChildren();
    gallery.dataset.layout = project.layout || "auto";
    project.images.forEach((source, index) => {
      const image = document.createElement("img");
      image.src = source;
      image.alt = `${project.title} 이미지 ${index + 1}`;
    ...
    document.body.classList.add("is-modal-open");
    if (typeof brandModal.showModal === "function") {
      brandModal.showModal();
    } else {
      brandModal.setAttribute("open", "");
    }
  };

  const closeBrandModal = () => {
    if (!brandModal.open) return;
    if (typeof brandModal.close === "function") {
      brandModal.close();
    } else {
      brandModal.removeAttribute("open");
      document.body.classList.remove("is-modal-open");
      lastBrandTrigger?.focus({ preventScroll: true });
    }
  };

  document.querySelectorAll("[data-brand-modal]").forEach((trigger) => {
    trigger.addEventListener("click", () => {
      openBrandModal(trigger.dataset.brandModal, trigger);
    });
  });
  ...
  brandModal.addEventListener("cancel", (event) => {
    event.preventDefault();
    closeBrandModal();
  });
  brandModal.addEventListener("close", () => {
    document.body.classList.remove("is-modal-open");
    lastBrandTrigger?.focus({ preventScroll: true });
  });
}
